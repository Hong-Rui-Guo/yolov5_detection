...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # judge whether detect num(in other words, check whether there is a food)
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if True:
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # transform xyxy to xywh(list) xywh
                        print('bounding box xywh:', end = '')
                        print(xywh)
